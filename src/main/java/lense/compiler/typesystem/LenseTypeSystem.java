...
public class LenseTypeSystem {
	...
	private static LenseTypeDefinition ANY = new FundamentalLenseTypeDefinition("lense.core.lang.Any", LenseUnitKind.Class, null);
	private static LenseTypeDefinition NOTHING = new FundamentalLenseTypeDefinition("lense.core.lang.Nothing", LenseUnitKind.Class, null);
	
	private static Map<TypeKey, LenseTypeDefinition> definitions = new HashMap<>();
	
	private static LenseTypeDefinition registerNative(LenseTypeDefinition definition) {
		definition.setNative(true);
		return register(definition);
	}
	
	public static LenseTypeDefinition register(LenseTypeDefinition definition) {
		definitions.put(new TypeKey(definition.getGenericParameters(), definition.getName()), definition);
		return definition;
	}

	static {
		LenseTypeDefinition any = register(ANY);

		LenseTypeDefinition nothing = registerNative(NOTHING);
		...
		register(new FundamentalLenseTypeDefinition("lense.core.math.Comparable", LenseUnitKind.Interface, any,
				 RangeTypeVariable.allRange("T", Variance.Covariant)));
		...
		LenseTypeDefinition function2 = register(new FundamentalLenseTypeDefinition("lense.core.lang.Function",
				LenseUnitKind.Interface, any, RangeTypeVariable.allRange("R", Variance.Invariant),
			    RangeTypeVariable.allRange("T", Variance.Invariant)));
		...
		LenseTypeDefinition iterable = register(new FundamentalLenseTypeDefinition("lense.core.collections.Iterable",
				LenseUnitKind.Interface, any, RangeTypeVariable.allRange("T", Variance.Covariant)));
		...
		LenseTypeDefinition maybe = registerNative(new FundamentalLenseTypeDefinition("lense.core.lang.Maybe",
				LenseUnitKind.Class, any, RangeTypeVariable.allRange("T", Variance.Covariant)));
		...
		RangeTypeVariable self = RangeTypeVariable.allRange("T", Variance.Covariant );
		LenseTypeDefinition tuple = register(new FundamentalLenseTypeDefinition("lense.core.collections.Tuple",
				LenseUnitKind.Class, any, RangeTypeVariable.allRange("V", Variance.ContraVariant), self));
		...
		LenseTypeDefinition keyValue = register(
				new FundamentalLenseTypeDefinition("lense.core.collections.KeyValuePair", LenseUnitKind.Interface, any,
					 RangeTypeVariable.allRange("K", Variance.ContraVariant),
					 RangeTypeVariable.allRange("V", Variance.Covariant)));
		...
		LenseTypeDefinition association = register(new FundamentalLenseTypeDefinition("lense.core.collections.Association", LenseUnitKind.Class,
				null, RangeTypeVariable.allRange("K", Variance.ContraVariant),
				RangeTypeVariable.allRange("V", Variance.Covariant)));
		...
		LenseTypeDefinition interval = register(new FundamentalLenseTypeDefinition("lense.core.math.Interval",
				LenseUnitKind.Class, any, RangeTypeVariable.allRange("T", Variance.Invariant))); // TODO
		// use
		// Comparable
	...
	public static LenseTypeDefinition specify(TypeDefinition definition, TypeVariable... genericParameters) {
		...
		if (definition.getGenericParameters().size() == genericParameters.length) {
			var match = true;
			for (int i = 0; i < genericParameters.length; i++ ) {
				if (!definition.getGenericParameters().get(i).equals(genericParameters[i])) {
					match = false;
					break;
				}
			}
			...
			if (match) {
				return (LenseTypeDefinition)definition;
			}
		}
	...
	private static LenseTypeDefinition specify(TypeDefinition definition, TypeDefinition... genericParametersCapture) {
		...
		for (int i = 0; i < definition.getGenericParameters().size(); i++) {
			...
			genericParameters[i] = RangeTypeVariable.upTo(gen.getSymbol(), gen.getVariance(), genericParametersCapture[i]);
