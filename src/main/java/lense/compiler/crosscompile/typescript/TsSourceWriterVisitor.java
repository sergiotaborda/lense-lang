 ...
 * 
 */
public class TsSourceWriterVisitor implements Visitor<AstNode> {

	private PrintWriter writer;
	private int iteratorIndex;
	private boolean isTypeClassMode;
	 ...
	 * @param writer
	 */
	public TsSourceWriterVisitor(PrintWriter writer) {
		this.writer = writer;
	}
	...
	public VisitorNext visitBeforeChildren(AstNode node) {
		try {
			...
			if (node instanceof PrimitiveStringNode primitiveString) {
				...
				writer.append("lense.core.lang.ts.NativeString.valueOfNative(").append("\"")
				...
				writer.append("lense.core.lang.ts.Primitives.").append(m.getCall().getName()).append("(");
				...
				if (typeVariable != null) {
					writer.append("/*" + operation
							+ (operation == ErasureOperation.CONVERSION ? ""
									: (box.isBoxingDirectionOut() ? " OUT" : " IN"))
							+ " to " + typeVariable.getTypeDefinition().getName() + "*/ ");

				} else {
					writer.append("/*" + operation + (operation == ErasureOperation.CONVERSION ? ""
							: (box.isBoxingDirectionOut() ? " OUT" : " IN")) + " to ? */ ");
					...
					if (name.equals("lense.core.lang.Binary")) {
						...
						if (n.isZero()) {
							...
							if (pos >= 0) {
								...
								writeInteger(decimalPart.toString());
								writer.append("))");
								
							} else {
								writer.append("lense.core.math.Rational.valueOf(lense.core.math.NativeNumberFactory.newInteger(");
								writeInteger(n.toString());
						...
						if ("Natural".equals(typeName)) {
							if (n.isZero()) {
								...
								writer.append("lense.core.math.NativeNumberFactory.newNatural(");
								writeInteger(n.toString());
								writer.append(")");
							}
						} else if ("Integer".equals(typeName)) {
							if (n.isZero()) {
								writer.append("lense.core.math.NativeNumberFactory.integerZero()");
							} else if (n.isOne()) {
								writer.append("lense.core.math.NativeNumberFactory.integerOne()");
							} else {
								writer.append("lense.core.math.NativeNumberFactory.newInteger(");
								writeInteger(n.toString());
								writer.append(")");
				...
				String reificationArgs = "lense.core.lang.ts.TsReifiedArguments.getInstance().addTypeByName(\""
				...
				writer.append(");");
				return VisitorNext.Siblings;
			} else if (node instanceof ClassTypeNode t) {

				this.isTypeClassMode = t.getProperty("typeClassMode", Boolean.class).orElse(false);

				int pos = t.getFullname().lastIndexOf('.');
				if (pos > 0) {
					writer.println("// Generated by Lense Compiler");
...
//				else if (t.isFinal() || t.getKind().isObject()) {  n/a
//					writer.print(" final ");
//				}
				...
				if (!t.getKind().isInterface() && !t.getKind().isEnhancement() && !t.getKind().isTypeClass()) {
					...
					if (t.getSuperType() != null
						...
						writer.print("lense.core.lang.ts.Base");
					}
				}

				var interfaceOrTypeClasss = new LinkedList<AstNode>();
				if (t.getInterfaces() != null) {
					interfaceOrTypeClasss.addAll(t.getInterfaces().getChildren());
				}
				if (t.getSatisfiedTypeClasses() != null) {
					interfaceOrTypeClasss.addAll(t.getSatisfiedTypeClasses().getChildren());
					this.isTypeClassMode = true;
				}

				if (!interfaceOrTypeClasss.isEmpty()) {
					...
					int count = 0;
					for (AstNode n : interfaceOrTypeClasss) {
						TypeNode tn = (TypeNode) n;
						writer.print(tn.getTypeVariable().getTypeDefinition().getName());
						count++;
						if (count > 0) {
							writer.print(" , ");
						}
				...
				} else if (t.getKind().isInterface() || t.getKind().isTypeClass()) {
				...
				if (t.getKind().isObject()) {
					...
					writer.append(" public static ").append(instanceName).append(" : ").append(className)
							.append(" = new ").append(className).append("();").println();

					// constructor factory
					//writer.append(" \n@lense.core.lang.java.Constructor(paramsSignature = \"\")").println();
					writer.append(" public static ").append(" constructor () : ").append(className).append("{\n return ")
					...
					//if (t.getProperty(JavalizePhase.AutoGenerateAsString, Boolean.class).orElse(false)) {
						writer.append(
								"public asString() :  lense.core.lang.String { return lense.core.lang.ts.NativeString.valueOfNative(\"")
				...
				if (varInfo.getTypeVariable().getTypeDefinition().getKind() == TsTypeKind.Primitive
						&& varInfo.getMaximum().isPresent() && varInfo.getMinimum().isPresent()) {
					...
					TsSourceWriterVisitor visitor = new TsSourceWriterVisitor(sp);
			...
			// }
				...
				if (defaultOption.isPresent()) {
					...
					writer.append("else {  throw new Error(\"Value is not completly covered\"); } ");
				}
...
//                    }
				...
				var constructor = n.getConstructor();

				if (constructor != null) {
					writer.print(constructor.getDeclaringType().getTypeDefinition().getName());
				} else if (n.getTypeVariable() == null) {
				...
				StringConcatenationNode s = (StringConcatenationNode) node;

				var deque = new LinkedList<AstNode>();
				inlineStringConcatenation(s, deque);

				writer.print(" lense.core.lang.ts.NativeString.valueOfNative(");
				...
				for (AstNode n : deque) {
					...
					if (n instanceof StringValue) {
						...
						} else if (n instanceof VariableReadNode) {
							VariableReadNode v = (VariableReadNode) n;

							if (v.getVariableInfo().getTypeVariable().getTypeDefinition().getName()
									.equals("lense.core.lang.String")) {

								TreeTransverser.transverse(v, this);
								writer.print(".toString()");
							} else {
								TreeTransverser.transverse(v, this);
								writer.print(".asString().toString()");
							}
							continue;
...
//				writer.println();
				...
				if (ctr.isNative()) {
					if (ctr.getBlock() != null) {
						TreeTransverser.transverse(ctr.getBlock(), this);
					} else {
						writer.println(
								"{ throw new Error(\"Native method not implemented natively\"); // native \n }");
					}
				...
				writer.println("\n");
				return VisitorNext.Siblings;
			} else if (node instanceof MethodDeclarationNode m) {

				if (m.isSatisfy() && !this.isTypeClassMode) {
					return VisitorNext.Siblings;
				}
...
//				}
	...
	private void writeInteger(String str) {

		var value = new BigInteger(str);
		
		if (isInIntRange(value)) {
			writer.append(str);
		} else if (isInLongRange(value)) {
	...
	private boolean isInIntRange(BigInteger value) {
		return value.compareTo(INT_MIN_VALUE) >= 0 && value.compareTo(INT_MAX_VALUE) <= 0;
	}
	...
	private void applyReboxOperator(PrintWriter writer, ArithmeticOperation operation, AstNode node) {
		...
		if (right.getTypeVariable().getTypeDefinition().getKind() == TsTypeKind.Primitive) {
	...
	private void writeMethodSignature(MethodDeclarationNode m) {
		...
		for (AstNode p : m.getParameters().getChildren()) {
			...
			if (typeVar instanceof CalculatedTypeVariable) {
				...
				writer.print(signatureNameOf(typeVar.getTypeDefinition().getName())); // TODO, recursive generic
																						// parameters
			}
		}
